...
Before launching long benchmark jobs, run `python cli.py preflight` to check disk space, CUDA availability, dataset cache status, and write permissions. The CLI will also run the same preflight checks automatically before `run` and `run_all` launches.
